...
class CFAviary(BaseAviary):
    ...
    CONTROLLER = 'pid'      #'mellinger' # specifies controller type 
    ...
    def sendRateCmd(self, thrust, roll_rate, pitch_rate, yaw_rate, timestep):
        """Send collective thrust and body rates as setpoints to the firmware."""
        self.command_queue += [['_sendRateCmd', [thrust, roll_rate, pitch_rate, yaw_rate, timestep]]]

    def _sendRateCmd(self, thrust, roll_rate, pitch_rate, yaw_rate, timestep):
        self.setpoint.thrust = thrust  # Check units expected by firmware!
        self.setpoint.attitudeRate.roll = roll_rate * self.RAD_TO_DEG  # Convert to deg/s if needed
        self.setpoint.attitudeRate.pitch = pitch_rate * self.RAD_TO_DEG
        self.setpoint.attitudeRate.yaw = yaw_rate * self.RAD_TO_DEG

        # Set setpoint modes for rate control
